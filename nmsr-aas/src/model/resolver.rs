...
impl RenderRequestResolver {
    ...
    async fn resolve_entry_textures(&self, entry: RenderRequestEntry) -> Result<ResolvedRenderEntryTextures> {
        ...
        match entry {
            ...
            RenderRequestEntry::TextureHash(skin_hash) => {
                ...
                let result = {
                    self.cache_manager.read().get_cached_skin(&skin_hash)?
                };
        ...
        Ok(ResolvedRenderEntryTextures {
            textures
        })
    }
